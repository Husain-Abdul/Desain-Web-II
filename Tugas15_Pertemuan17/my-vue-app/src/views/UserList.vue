        ...
        <div v-else class="overflow-x-auto">
            <table class="w-full table-auto border-separate border-spacing-y-2">
                <thead class="bg-gradient-to-r from-blue-100 to-blue-200 rounded-t-lg">
                    <tr class="text-center font-bold text-blue-700 uppercase tracking-wider">
                        <th class="px-6 py-3 rounded-tl-lg">Avatar</th>
                ...
                <tbody>
                    <tr v-for="user in users" :key="user.id" class="bg-white shadow-md hover:scale-[1.01] hover:shadow-lg transition-all duration-150 rounded-lg">
                        ...
                        <td class="px-4 py-3 space-x-2">
                            <button @click="editUser(user.id)" class="text-white bg-blue-600 px-4 py-1 rounded">Edit</button>
                            <button @click="deleteUser(user.id)" class="text-white bg-red-600 px-4 py-1 rounded">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
